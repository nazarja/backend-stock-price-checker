  ...
  app.route('/api/stock-prices')
    .get(function (req, res){
      ...
      function singleStock() {
        ...
        fetch(`https://api.iextrading.com/1.0/stock/${stock}/price`, { method: 'GET', mode: 'no-cors' })
        .then(response => {
      ...
      function doubleStock() {
        ...
        fetch(`https://api.iextrading.com/1.0/stock/market/batch?symbols=${stock[0]},${stock[1]}&types=price`, { method: 'GET', mode: 'no-cors' })
        .then(response => {
            return response.json();
        })
        .then(result => {
      ...
      function Mongo(num) {
        objectsArray.forEach((item, i) => {
          ...
          MongoClient.connect(DATABASE, (err, database) => {
              ...
              db.findOne({ ticker: item.stock}, (err, docs) => {
                if (err) console.log(err);

                // If if doesnt exist create
                if (docs === null) {
                    console.log('Inserting New Entry');
                    db.insertOne({ ticker: item.stock, likes: (likes ? [ip] : []), numlikes: 0}, (err, doc) => {
                        if (err) console.log(err);
                    })
                }
                // If it does exist - update it
                  ...
                  if (likes && update) {
                    db.findOneAndUpdate( {ticker: item.stock}, {$push: {likes: ip}, $inc: { numlikes: 1 }}, (err, doc) => {
                       if (err) console.log(err);
                       objectsArray[i].likes = 1;
